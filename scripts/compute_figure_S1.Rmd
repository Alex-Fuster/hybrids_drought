...
library(dplyr)
sites_plot <- sites %>%
  filter(species %in% c("pseudomajus", "striatum"))

plot_pyr <- ggplot(world) +
  geom_sf() +
  geom_point(
    data = sites_plot,
  ...
  coord_sf(xlim = c(-3.17, 3.49), ylim = c(41.10, 44.28), expand = FALSE) +
  scale_color_manual(
    name = "Varieties",
    values = c(pseudomajus = "#990099", striatum = "gold"),
    labels = c(pseudomajus = "A. m. var. pseudomajus", striatum = "A. m. var. striatum"),
    breaks = c("pseudomajus", "striatum")
  )
